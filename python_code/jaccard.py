...
def cal_all_correlation(contig_list, contig_dict, barcode_dict, thresh1, thresh2, weight_index):
    corr_map = []
    tmp_barcode_dict = barcode_dict
    for contig1 in contig_list:  #contig是所有的，都要遍历一遍
        if(contig1[0] in contig_dict.keys()):
        
            contig1_index = contig_list.index(contig1)    
            candidate_contig_dict = get_candidate_contig(contig1[0], contig_dict, tmp_barcode_dict, thresh1, thresh2)
            #得到合法的contig，即contig的barcode要出现多次
            #tmp_barcode_dict = update_twodimdict(contig1[0], contig_dict, tmp_barcode_dict)

            for contig2 in contig_list:
                contig2_index = contig_list.index(contig2)
                #if contig2_index >=contig1_index:
    
                if contig2_index > contig1_index and whether_exist(contig2[0], candidate_contig_dict) == True:#单向判断,2>1,contig2要符合规则
                    #tmp_corr = 9
                    tmp_corr = cal_a_correlation(contig1, contig2, contig_dict, weight_index)
...
def cal_a_correlation(contig1, contig2, contig_dict, weight_index):

    tmp_corr =[]
    head_to_head = cal_corr_ht(contig1, contig2, 1, 1, contig_dict, weight_index)
    tmp_corr.append(head_to_head)
    head_to_tail = cal_corr_ht(contig1, contig2, 1, 0, contig_dict, weight_index)
    tmp_corr.append(head_to_tail)
    tail_to_head = cal_corr_ht(contig1, contig2, 0,1, contig_dict, weight_index)
    tmp_corr.append(tail_to_head)
    tail_to_tail = cal_corr_ht(contig1, contig2, 0, 0 , contig_dict, weight_index)
    tmp_corr.append(tail_to_tail)

    max_corr = max(tmp_corr)
    return max_corr
    
def cal_corr_ht(contig1, contig2, ht_index1, ht_index2, contig_dict, weight_index):
    """
    ...
    if ht_index1==1 and ht_index2==1:
        ...
        contig2_bin = ContigBin(contig2[0], contig2[1])  #计算2的bin
        contig2_name = contig2[0]
        start2 = contig2_bin.get_head()#开始位置
        
        contig1_barcode_dict = get_bin_barcodeset(contig1_name,binsize, start1, contig_dict )  #算1,2
        
        contig2_barcode_dict = get_bin_barcodeset(contig2_name, binsize, start2, contig_dict)
        corr = cal_jaccard(contig1_barcode_dict, contig2_barcode_dict, weight_index)#
    elif ht_index1 ==1 and ht_index2==0:
        contig1_bin = ContigBin(contig1[0], contig1[1])
        contig1_name =contig1[0]
        binsize = contig1_bin.binsize
        start1 = contig1_bin.get_head()
        contig2_bin = ContigBin(contig2[0], contig2[1])
        contig2_name = contig2[0]
        start2 = contig2_bin.get_tail()
...
contig_dict_file = sys.argv[2]
contig_dict = load_dict(contig_dict_file)
barcode_dict_file = sys.argv[3]
barcode_dict = load_dict(barcode_dict_file)
thresh1 = 2
thresh2 = 2
weight_index = 1
corr_map_file=sys.argv[4]
corr_map_list = cal_all_correlation(tmp_contig_list, contig_dict, barcode_dict, thresh1, thresh2, weight_index)
output_corr_map(corr_map_list, corr_map_file)
